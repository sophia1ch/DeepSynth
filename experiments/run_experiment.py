...
COMMUTATIVE_FUNCS = {"OR", "AND", "TOUCHING", "EXACTLY_2", "AT_LEAST_2", "ODD_2", "EVEN_2"}
    ...
    while (search_time + evaluation_time < timeout and nb_programs < total_number_programs):
        ...
        if is_match:
            ...
            if len(found_programs) >= 10:
                logging.debug("\tFound {} programs, stopping search.".format(len(found_programs)))
                break
